...
class CoreSwitch(Widget):
    ...
    :attr:`state` is a :class:`~kivy.properties.BooleanProperty`
    '''
    ...
    def compute_switch_size(self, *args, state=None):
    ...
    def compute_switch_pos(self, *args, state=None):
        if state is None:
            size = self.switch_size
                
        else:
            size = self.compute_switch_size(state=state)
    ...
    def on_state(self, instance, value):
        anim = Animation(
            ...
            switch_pos=self.compute_switch_pos(state=value),
            duration=0.2,
            t='out_quad'
        )
        anim.start(self)
